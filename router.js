...
router.post('/getUser', async (req, res) => {
    ...
    const usersDb = db.use("user");             // select the users DB

    const result = await usersDb.find({          // query CouchDB
        selector: { user: "vadim" },
        limit: 1
    });

    if(result.docs[0].user===username){
        console.log("the candidate: "+ username + " correct")
    }
    else console.log("the candidate: " +username+" is not saved in our db")




    res.json(result.docs[0]);                                  // return all users
});
